...
export async function proxy(request: NextRequest) {
  ...
  if (supabaseResponse.headers.get('location') || supabaseResponse.status !== 200) {
    // Pastikan status 200 adalah NextResponse.next(), selain itu berarti redirect/error
  ...
  supabaseResponse.cookies.getAll().forEach((cookie) => {
    intlResponse.cookies.set(cookie.name, cookie.value, {
      ...cookie,
      // pastikan option cookie disalin
    });
  });
...
export const config = {
  ...
  matcher: ['/((?!api|_vercel|_next/static|_next/image|.*\\..*).*)']
};
