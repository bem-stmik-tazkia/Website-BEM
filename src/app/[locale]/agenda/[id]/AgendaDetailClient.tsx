...
function AgendaDetailClientContent({ agenda: rawAgenda, participantCount }: { agenda: AgendaKegiatan, participantCount: number }) {
  const t = useTranslations("AgendaDetail");
  const locale = useLocale();
  const searchParams = useSearchParams();
  const router = useRouter();
  const from = searchParams.get("from");

  const { data: translatedArr } = useTranslatedList([rawAgenda], "agenda_kegiatan", locale, ["title", "description", "location", "category"]);
  const agenda = translatedArr[0] || rawAgenda;
          ...
          >
            <div className={`flex items-center gap-3 md:gap-4 w-full sm:w-auto ${isLive ? 'text-white' : 'text-on-surface'}`}>
              ...
              <div>
                <h3 className="font-bold text-base md:text-lg">
                  {isLive ? t("eventOngoing") : hasRegistration ? t("ticketRegistration") : t("attendanceInfo")}
                </h3>
                ...
                {hasRegistration && (
                   <p className={`text-xs mt-1 font-bold ${isQuotaFull ? 'text-red-500' : isLive ? 'text-white' : 'text-primary'}`}>
                     {maxQuota !== null && maxQuota !== undefined 
                       ? (isQuotaFull ? t("quotaFull") : `${t("remainingQuota")}: ${remainingQuota} ${t("of")} ${maxQuota}`)
                       : t("unlimitedQuota")}
                     {` (${t("totalRegistrants")}: ${participantCount})`}
                   </p>
                )}
              </div>
            </div>
            ...
            <div className="flex flex-col sm:flex-row w-full sm:w-auto gap-3">
              {agenda.online_link && (
                ...
                >
                  {isLive ? t("enterLive") : t("onlineLink")} <FiExternalLink size={16} />
                </a>
              )}
              {hasRegistration && (
                <Link 
                ...
                >
                  {isQuotaFull ? t("quotaFull") : t("registerNow")}
                </Link>
              )}
            </div>
          </motion.div>
        ...
        <div className="flex flex-col gap-6 md:gap-8 lg:col-span-2">
          ...
          {agenda.speakers && agenda.speakers.length > 0 && (
            <div className="lg:hidden flex flex-col gap-4 mb-2">
              <span className="text-[10px] font-bold text-secondary uppercase tracking-wider block bg-secondary/10 w-fit px-3 py-1.5 rounded-full">{t("featuringSpeakers")}</span>
                ...
                <div className="flex items-center gap-2">
                  {isSelectionMode ? (
                    <>
                      ...
                      >
                        {isDownloading ? (
                          <><div className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"></div> {t("downloading")}</>
                        ) : (
                          <><FiDownload size={14} /> {t("download")} ({selectedPhotos.size})</>
                        )}
                      </button>
                    </>
                  ) : (
                    <>
                      <button onClick={handleDownloadAll} disabled={isDownloading} className="text-xs md:text-sm font-semibold px-3 py-1.5 rounded-lg border border-outline-variant/50 hover:bg-surface-variant transition-colors flex items-center gap-1.5 disabled:opacity-50">
                        {isDownloading ? <div className="w-3 h-3 border-2 border-primary/40 border-t-primary rounded-full animate-spin"></div> : <FiDownload size={14} />} {t("all")}
                      </button>
                      <button onClick={() => setIsSelectionMode(true)} disabled={isDownloading} className="text-xs md:text-sm font-bold bg-secondary-container text-secondary px-3 py-1.5 rounded-lg hover:bg-secondary/20 transition-colors flex items-center gap-1.5 disabled:opacity-50">
                        <FiCheckSquare size={14} /> {t("select")}
                      </button>
                    </>
                  )}
                </div>
        ...
        <div className="hidden lg:flex flex-col gap-6 lg:col-span-1">
          {agenda.speakers && agenda.speakers.length > 0 && (
            <div className="flex flex-col gap-4">
              <span className="text-[10px] font-bold text-secondary uppercase tracking-wider bg-secondary/10 px-3 py-1.5 rounded-full w-fit">{t("featuringSpeakers")}</span>
              {agenda.speakers.map((speaker, idx) => (
