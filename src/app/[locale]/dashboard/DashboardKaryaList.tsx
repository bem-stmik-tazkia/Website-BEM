...
export default function DashboardKaryaList({ initialKaryaList }: { initialKaryaList: any[] }) {
  ...
  const submitDeleteRequest = async () => {
    ...
    if (!error) {
      ...
      toast(t("alertRequestFail"), "error");
    }
    setIsSubmitting(false);
  };
  ...
  if (!karyaList || karyaList.length === 0) {
    return (
      ...
      >
        ...
        <h3 className="text-xl font-bold text-on-surface mb-3">{t("emptyTitle")}</h3>
  ...
  const TABS = [
    ...
    { id: "approved", label: t("tabApproved"), count: counts.approved },
    { id: "rejected", label: t("tabRejected"),count: counts.rejected },
  ];

  return (
    <div className={`transition-opacity duration-300 ${isTranslating ? "opacity-70" : "opacity-100"}`}>
      {/* ── Tabs & Filters ────────────────────────────────────────────────── */}
      <div className="flex flex-col sm:flex-row gap-4 mb-6 justify-between items-start sm:items-center w-full">
        <LayoutGroup>
          <div className="flex overflow-x-auto gap-2 pb-2 sm:pb-0 w-full sm:w-auto scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
            {TABS.map((tab) => (
              <motion.button
                key={tab.id}
                ...
                whileTap={{ scale: 0.95 }}
                className={`relative px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-colors shrink-0 ${
                  activeTab === tab.id
                    ? "text-white"
        ...
        <select
          ...
          className="w-full sm:w-auto px-4 py-2 rounded-xl text-sm font-bold bg-surface-variant/30 border border-outline-variant/30 outline-none focus:ring-2 focus:ring-primary text-on-surface transition-shadow shrink-0"
        >
          <option value="all">{t("catAll")}</option>
          <option value="Technology">{t("catWeb")}</option>
          <option value="Programming">{t("catMobile")}</option>
          <option value="Research">{t("catResearch")}</option>
          <option value="IoT">{t("catIoT")}</option>
          <option value="Multimedia">{t("catDesign")}</option>
        </select>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {filteredList.length === 0 ? (
            ...
            >
              <p className="text-sm text-on-surface-variant font-medium">{t("emptyCategory")}</p>
            </motion.div>
          ) : (
                    ...
                    <div className="flex-1 min-w-0 flex flex-col">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[10px] font-bold px-2 py-1 rounded-md bg-surface-variant text-on-surface-variant uppercase tracking-wider shrink-0">
                          {getCategoryLabel(karya.category, t)}
                        </span>
                      </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pt-2 border-t border-outline-variant/10">
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[11px] font-medium text-on-surface-variant/70">
                        {t("uploaded")} {new Date(karya.created_at).toLocaleDateString(locale)}
                      </span>
                    ...
                    <div className="shrink-0 flex flex-col gap-1.5">
                      ...
                      {karya.status === 'approved' && (
                        <div className="flex flex-col gap-1.5 w-fit items-end">
                          <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-green-50 text-green-600 font-semibold text-xs border border-green-100">
                            <FiCheckCircle size={14} /> {t("approved")}
                          </div>
                          {karya.pending_edits && !karya.edit_reject_reason && (
                            <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-amber-50 text-amber-600 font-semibold text-xs border border-amber-100 w-fit">
                              <FiClock size={14} /> {t("editPending")}
                            </div>
                          )}
                          {karya.edit_reject_reason && (
                            ...
                            >
                              <FiAlertCircle size={14} /> {t("editRejected")}
                            </button>
                          )}
                          {karya.deletion_reject_reason && (
                            ...
                            >
                              <FiAlertCircle size={14} /> {t("deleteRejected")}
                            </button>
                          )}
                        </div>
                      )}
                      {karya.status === 'rejected' && (
                        <div className="flex flex-col gap-1.5 w-fit items-end">
                          <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-red-50 text-red-600 font-semibold text-xs border border-red-100 w-fit">
                            <FiAlertCircle size={14} /> {t("rejected")}
                          </div>
                          {karya.reject_reason && (
                            <button
                      ...
                      {karya.status === 'deletion_pending' && (
                        <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-purple-50 text-purple-600 font-semibold text-xs border border-purple-100 w-fit">
                          <FiClock size={14} /> {t("deletePending")}
                        </div>
                      )}
                      {karya.status === 'deleted' && (
                        <div className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-gray-100 text-gray-700 font-semibold text-xs border border-gray-300 w-fit shadow-sm">
                          <FiTrash2 size={14} /> {t("deletedByAdmin")}
                        </div>
                      )}
                    </div>
                  </div>
          ...
          >
            ...
            >
              <div className="p-6 border-b border-outline-variant/20">
                <h3 className="text-xl font-bold text-on-surface">{t("requestDeleteTitle")}</h3>
        ...
        {confirmDeleteData && (
          ...
          >
            ...
            >
              ...
              <h3 className="text-xl font-bold text-on-surface mb-2">{t("confirmDeleteTitle")}</h3>
              <p className="text-sm text-on-surface-variant mb-6 leading-relaxed">
                {t("confirmDeleteDesc", { title: confirmDeleteData.title })}
              </p>
              <div className="flex gap-3 w-full">
                <button
