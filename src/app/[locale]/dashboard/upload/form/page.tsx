...
export default function UploadKaryaPage() {
  ...
  const tKarya = useTranslations("UploadKarya");
  ...
  let techStackLabel = isKTI ? t("techKTI") : (typeParam === "IoT" ? t("techIoT") : (isLainnya ? t("techLainnya") : t("techDefault")));
  ...
  let liveUrlLabel = isKTI ? t("urlKTI") : (typeParam === "Programming" ? t("urlProg") : (typeParam === "IoT" ? t("urlIoT") : (isLainnya ? t("urlLainnya") : t("urlDefault"))));
  let featuresLabel = isKTI ? t("featKTI") : (isLainnya ? t("featLainnya") : (typeParam === "IoT" ? t("featIoT") : t("featDefault")));
  let featuresDesc = isKTI ? t("featDescKTI") : (isLainnya ? t("featDescLainnya") : (typeParam === "IoT" ? t("featDescIoT") : t("featDescDefault")));
  
  let techSectionTitle = isKTI ? t("sectionTechKTI") : (typeParam === "IoT" ? t("sectionTechIoT") : (isLainnya ? t("sectionTechLainnya") : t("sectionTechDefault")));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (fields.length > 0) {
      ...
      toast(t("toastErrorFields"), "error");
  ...
  return (
    <div className="w-full pt-8 md:pt-12 pb-24 md:pb-12">
        ...
        <form noValidate onSubmit={handleSubmit} className="bg-surface p-6 md:p-8 rounded-3xl shadow-sm border border-outline-variant/20 space-y-8">
          ...
          <div>
            ...
            <div className="space-y-4">
              ...
              <div>
                <label className={labelClass}>{t("category")} <span className="text-red-400">*</span></label>
                <select id="input-category" name="category" value={formData.category} onChange={handleInput} className={`${getInputClass("input-category")} !bg-surface-variant/60 !text-on-surface-variant/70 cursor-not-allowed`} disabled>
                  ...
                  {KATEGORI_OPTIONS.map(opt => <option key={opt.id} value={opt.id}>{tKarya(opt.id === 'Technology' ? 'catTechTitle' : opt.id === 'Programming' ? 'catProgTitle' : opt.id === 'Research' ? 'catResTitle' : opt.id === 'IoT' ? 'catIotTitle' : 'catMultiTitle')}</option>)}
                </select>
                <p className="text-xs text-on-surface-variant mt-2 font-medium">{t("categoryHint")}</p>
              </div>
              <div>
                <div className="flex justify-between items-center mb-1.5">
          ...
          <div id="section-tech">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {showGithub && (
                  <div>
                    <label htmlFor="input-github-url" className={labelClass}>{t("githubUrl")}</label>
          ...
          <div id="section-features">
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={addFitur} className="flex items-center gap-1.5 text-xs font-bold text-[var(--color-primary)] hover:text-[var(--color-secondary)] transition-colors">
                <FiPlus size={14} /> {isKTI ? t("addPoint") : (isLainnya ? t("addDetail") : t("addFeature"))}
              </button>
            </div>
              ...
              {formData.features.map((fitur, i) => (
                <div key={i} className="bg-surface-variant/20 rounded-2xl p-4 border border-outline-variant/20">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-xs font-bold text-on-surface-variant/70 uppercase tracking-wide">{isKTI ? t("point") : (isLainnya ? t("detail") : t("feature"))} {i + 1}</p>
                  ...
                  <div className="flex-1 space-y-3">
                    <input type="text" value={fitur.title} onChange={(e) => handleFitur(i, "title", e.target.value)} placeholder="..." className={getInputClass(`input-feature-${i}-title`)} id={`input-feature-${i}-title`} />
                    <textarea rows={2} value={fitur.desc} onChange={(e) => handleFitur(i, "desc", e.target.value)} placeholder="..." className={`${getInputClass("")} resize-none`} />
                  </div>
                </div>
              ))}
            </div>
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {formData.team.length < 5 && <button type="button" onClick={addAnggota} className="flex items-center gap-1.5 text-xs font-bold text-[var(--color-primary)] hover:text-[var(--color-secondary)] transition-colors"><FiPlus size={14} /> {t("addMember")}</button>}
            </div>
            <div className="space-y-3">
                ...
                <div key={i} className="bg-surface-variant/20 rounded-2xl p-4 border border-outline-variant/20 space-y-3">
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <input type="text" value={anggota.role} onChange={e => handleTim(i, "role", e.target.value)} placeholder="Peran" className={getInputClass(`input-team-${i}-name`)} />
                    </div>
                  </div>
                </div>
              ))}
          ...
          <div>
            <div className="flex items-center justify-between mb-4">
              ...
              {formData.gallery.length < 10 && <button type="button" onClick={addGallery} className="flex items-center gap-1.5 text-xs font-bold text-[var(--color-primary)] hover:text-[var(--color-secondary)] transition-colors"><FiPlus size={14} /> {t("addPhoto")}</button>}
            </div>
            <div className="space-y-3">
              {formData.gallery.map((gal, i) => (
                <div key={i} className="rounded-2xl p-4 border border-outline-variant/20 bg-surface-variant/20 flex flex-col sm:flex-row gap-4 items-start">
                  <div id={`input-gallery-${i}-url`} className={`shrink-0 p-1 rounded-2xl transition-all ${invalidFields.includes(`input-gallery-${i}-url`) ? 'border-2 border-red-500 bg-red-50/50 shadow-[0_0_0_4px_rgba(239,68,68,0.15)]' : 'border-2 border-transparent'}`}>
                    <ImageUpload value={gal.url} onChange={(url) => handleGallery(i, "url", url)} />
                  </div>
                  <div className="flex-1 flex flex-col gap-3 justify-center">
                    <div className="flex items-center justify-between"><p className="text-xs font-bold text-on-surface-variant/70 uppercase tracking-wide">{t("photo")} {i + 1}</p><button type="button" onClick={() => removeGallery(i)} className="text-gray-300 hover:text-red-400 transition-colors"><FiTrash2 size={14} /></button></div>
                    <input type="text" value={gal.caption} onChange={e => handleGallery(i, "caption", e.target.value)} placeholder={t("caption")} className={getInputClass("")} />
                  </div>
                </div>
              ))}
            </div>
